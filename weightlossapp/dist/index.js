...
async function generateDailyInsight(profile, recentWeightEntries = [], todaysMeals = []) {
  const weightLost = recentWeightEntries.length > 0 ? profile.currentWeight - recentWeightEntries[recentWeightEntries.length - 1].weight : 0;
...
async function generateMotivation(profile, context) {
...
async function answerWeightLossQuestion(question, profile) {
  const profileContext = profile ? `User profile: ${profile.age} year old ${profile.gender}, ${profile.height} inches tall, current weight ${profile.currentWeight} lbs, target ${profile.targetWeight} lbs, ${profile.activityLevel} activity level.` : "";
...
async function generateMealSuggestions(calorieTarget, mealType, dietaryPreferences = []) {
  const preferencesText = dietaryPreferences.length > 0 ? `Dietary preferences: ${dietaryPreferences.join(", ")}.` : "";
...
var appRouter = router({
  ...
  ai: router({
    getDailyInsight: publicProcedure.input(
      z2.object({
        profile: z2.object({
          name: z2.string(),
          currentWeight: z2.number(),
          targetWeight: z2.number(),
          height: z2.number(),
          age: z2.number(),
          gender: z2.string(),
          activityLevel: z2.string(),
          dailyCalories: z2.number().optional()
        }),
        recentWeightEntries: z2.array(
          z2.object({
            weight: z2.number(),
            date: z2.date()
          })
        ).optional(),
        todaysMeals: z2.array(
          z2.object({
            name: z2.string(),
            calories: z2.number(),
            protein: z2.number(),
    ...
    getMotivation: publicProcedure.input(
      z2.object({
        profile: z2.object({
          name: z2.string(),
          currentWeight: z2.number(),
    ...
    askQuestion: publicProcedure.input(
      z2.object({
        question: z2.string(),
        profile: z2.object({
          name: z2.string(),
    ...
    getMealSuggestions: publicProcedure.input(
      z2.object({
        calorieTarget: z2.number(),
        mealType: z2.string(),
        dietaryPreferences: z2.array(z2.string()).optional()
      })
    ).mutation(async ({ input }) => {
